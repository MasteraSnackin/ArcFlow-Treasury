...
```markdown
# ArcFlow Treasury – Architecture

ArcFlow Treasury uses a three‑layer architecture with Arc as the primary execution environment and Circle as the global settlement fabric.

## System Overview

- **On‑chain (Arc):** Smart contracts deployed on Arc testnet define escrows, vesting streams, and payout batches. Arc is an L1 built by Circle specifically for stablecoin‑native finance, with USDC as native gas, low and predictable fees, and fast finality via Malachite consensus.
- **Off‑chain (Backend):** A Node/Express (or similar) backend exposes a REST API for the frontend and a worker that listens for events like `PayoutInstruction` from Arc. This worker is responsible for mapping on‑chain obligations into Circle API calls (Wallets, Gateway, CCTP) and persisting payout status.
- **Frontend (Dashboard):** A React/Next UI shows aggregated treasury state – escrows, streams, and payout batches – and surfaces status from both Arc and Circle. Users never manually bridge assets or switch networks; they operate on Arc, and settlement is delegated to Circle in the background.

## Payout Flow (Arc + Circle)

1. Treasury operator creates a payout batch in the ArcFlow UI.
2. Frontend calls backend `POST /batches` with recipients, amounts, and destination chain labels.
3. Backend calls the `PayoutRouter` contract on Arc testnet to create the batch.
4. Arc emits one `PayoutInstruction` event per recipient; each includes `batchId`, `recipient`, `amount`, and destination metadata.
5. Backend worker subscribes to these events via an Arc RPC endpoint and:
   - Builds a Circle Wallets / Gateway request for each instruction.
   - Uses CCTP when a native cross‑chain USDC/EURC transfer is required.
6. Circle processes the payout, moving USDC/EURC to the requested destination chain/account.
7. Circle notifies the backend via webhooks or polling; backend updates batch entry status in its database.
8. Frontend queries `GET /batches/:id` and displays status (QUEUED / COMPLETED / FAILED) per recipient.

Arc remains the canonical record of obligations and batch definitions; Circle is the canonical source of truth for off‑chain and cross‑chain settlement results.

## Escrow and Vesting Flows

- **Escrow:**
  - User creates an escrow (payer → payee) on ArcFlow UI.
  - Frontend calls backend, backend calls `ArcFlowEscrow` on Arc.
  - Contract holds USDC/EURC until either:
    - Expiry + no dispute → auto‑release to payee.
    - Dispute / resolution path → funds released per contract rules.
- **Vesting:**
  - User defines a stream (employer → employee) with start, cliff, end, and total amount.
  - `ArcFlowStreams` contract tracks accumulated, withdrawn, and remaining balances.
  - UI reads from Arc to show vested/withdrawable amounts and allows withdrawals or revocations according to the contract.

## Arc‑specific Design Notes

- **Stablecoin gas:** All user interactions with ArcFlow contracts pay gas in USDC/EURC on Arc, which keeps treasury operations denominated in the same units as obligations and reduces volatility in operational costs.
- **Finality:** Arc’s Malachite‑based consensus provides fast finality, which is important for flows like payroll and escrow release where users expect immediate certainty.
- **Multichain alignment:** Arc is designed as a neutral coordination layer in a multichain world. ArcFlow assumes Arc holds the obligation graph, while Circle’s Gateway and CCTP handle moving stablecoins into and out of Arc as needed.

(You can merge this with your existing detailed diagrams if you already have them.)
